...
const NCsClientes = () => {
  ...
  const { data: obras = [] } = useQuery({
    queryKey: ["obras", formData.filial_id, formData.cliente_id],
    queryFn: async () => {
      let query = supabase
        .from("obras")
        .select("id, nome")
  ...
  const { data: editObras = [] } = useQuery({
    queryKey: ["obras-edit", editFormData.filial_id, editFormData.cliente_id],
    queryFn: async () => {
      let query = supabase
        .from("obras")
        .select("id, nome")
  ...
  // Fetch tipos de NC para clientes
  const { data: tiposNC = [] } = useQuery({
    queryKey: ["tipos-nc-cliente"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("tipos_nc")
        .select("id, nome")
  ...
  // Fetch NCs de clientes
  const { data: ncs = [], isLoading } = useQuery({
    queryKey: ["ncs-clientes", filterFilial, filterStatus],
    queryFn: async () => {
      let query = supabase
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    if (!editFormData.titulo || !editFormData.descricao || !editFormData.cliente_id || !editFormData.filial_id) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.titulo || !formData.descricao || !formData.cliente_id || !formData.filial_id) {
      toast.error("Preencha todos os campos obrigatórios");
      return;
    }
  ...
  const filteredNCs = ncs.filter(
    (nc) =>
      ...
      nc.cliente?.nome?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">NCs de Clientes</h1>
          <p className="text-muted-foreground">
            Gerencie não conformidades relacionadas a clientes
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => exportNCsPDF(filteredNCs, 'NCs de Clientes')}>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md max-h-[85vh] flex flex-col overflow-hidden">
            <DialogHeader className="flex-shrink-0">
              <DialogTitle>Registrar NC de Cliente</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedNC.cliente?.nome}</p>
                </div>
                <div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-md max-h-[85vh] flex flex-col overflow-hidden">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>Editar NC de Cliente</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleEditSubmit} className="flex-1 overflow-y-auto space-y-3 pr-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setEditFormData({ ...editFormData, filial_id: value })
                  }
                >
                  <SelectTrigger className="h-9">
                    <SelectValue placeholder="Selecione" />
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setEditFormData({ ...editFormData, cliente_id: value })
                }
              >
                <SelectTrigger className="h-9">
                  <SelectValue placeholder="Selecione o cliente" />
